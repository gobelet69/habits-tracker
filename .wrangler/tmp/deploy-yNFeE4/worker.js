...
var worker_default = {
  async fetch(req, env) {
    ...
    if (url.pathname === "/habits/login" && method === "POST") {
      ...
      await env.AUTH_DB.prepare("INSERT INTO sessions (id, username, role, expires) VALUES (?, ?, ?, ?)").bind(newSess, dbUser.username, dbUser.role, Date.now() + 864e5).run();
    ...
    if (url.pathname === "/habits/api/reorder" && method === "POST") {
      ...
      const { results: habits2 } = await env.DB.prepare("SELECT * FROM habits WHERE username = ? ORDER BY created_at ASC").bind(user.username).all();
      const idx = habits2.findIndex((h) => h.id === habitId);
      if (idx !== -1) {
        let targetTs = null;
        if (direction === "up" && idx > 0) targetTs = habits2[idx - 1].created_at - 1;
        if (direction === "down" && idx < habits2.length - 1) targetTs = habits2[idx + 1].created_at + 1;
        if (targetTs !== null) {
...
function renderNav(active) {
  return `<div style="display:flex;gap:10px">
    <a href="/habits" class="nav-link ${active === "dash" ? "active" : ""}"><span style="font-size:1.2em">\u{1F4C5}</span> Tracker</a>
    <a href="/habits/history" class="nav-link ${active === "hist" ? "active" : ""}"><span style="font-size:1.2em">\u{1F4DA}</span> History</a>
...
function renderLogin() {
  ...
  <body style="display:flex;justify-content:center;align-items:center;height:100vh">
    <div class="card" style="width:350px;text-align:center">
      <h2 style="margin-bottom:5px">\u{1F3AF} Habit Tracker</h2>
...
function renderHistory(user, habits, logs) {
  const historyData = {};
  logs.forEach((l) => {
    const yr = l.date.slice(0, 4), mo = l.date.slice(5, 7);
    if (!historyData[yr]) historyData[yr] = {};
    if (!historyData[yr][mo]) historyData[yr][mo] = {};
    historyData[yr][mo][l.habit_id] = (historyData[yr][mo][l.habit_id] || 0) + 1;
  });
  ...
  return `<!DOCTYPE html><html lang="en"><head><title>History</title><style>${CSS}</style></head><body>
    ...
    ${years.length === 0 ? '<div class="card">No history available yet.</div>' : years.map((yr) => `<div class="card"><h3>\u{1F4C5} ${yr} Breakdown</h3><div style="overflow-x:auto"><table><tr><th style="background:#121212">Habit</th>${monthNames.map((m) => `<th>${m}</th>`).join("")}<th>Total</th></tr>${habits.map((h) => {
    let yTot = 0;
    const mCols = months.map((m) => {
      const count = historyData[yr]?.[m]?.[h.id] || 0;
      yTot += count;
      const alpha = count / 30;
      return `<td style="background:rgba(76, 175, 80, ${alpha}); color:${count > 0 ? "#fff" : "#444"}">${count}</td>`;
    }).join("");
    return `<tr><td style="font-weight:bold">${h.name}</td>${mCols}<td style="font-weight:bold;color:var(--p)">${yTot}</td></tr>`;
...
function renderDash(user, habits, logs) {
  ...
  const habitsWithData = habits.map((h) => {
    ...
    let last30 = 0, d30 = /* @__PURE__ */ new Date();
    for (let i = 0; i < 30; i++) {
  ...
  return `<!DOCTYPE html><html lang="en"><head><title>Habits</title><style>${CSS}</style><script src="https://cdn.jsdelivr.net/npm/chart.js"><\/script></head><body>
    ...
    <div class="card">
      <div class="row"><h3>\u{1F4CA} 14-Day Grid</h3><form onsubmit="event.preventDefault();addHabit(this)" style="display:flex;gap:5px"><input type="text" name="name" placeholder="New Habit..." required><button>Add</button></form></div>
      <div style="overflow-x:auto"><table>
        ...
        ${habitsWithData.map((h) => `<tr>
          <td style="font-weight:bold;text-align:left;display:flex;justify-content:space-between;align-items:center;border:none">
            <span>${h.name} ${h.streak >= 3 ? `<span class="streak">\u{1F525} ${h.streak}d</span>` : ""}</span>
            <div style="white-space:nowrap">
              ...
              <button class="ctrl-btn del" onclick="delHabit('${h.id}', '${h.name}')" title="Delete">\u{1F5D1}</button>
            </div>
          </td>
    ...
    <div class="stats-grid">
      ...
      <div class="card">
        <h3>\u{1F4C8} Daily Momentum (14 Days)</h3>
